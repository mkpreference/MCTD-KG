...
def csv_gen(p2, file_path="./2_4_1/batch_4_1.csv", custom_id=None):
    """
    ...
    prompt1 = f'''
            Based on the following explanation of relation types and example  , extract Entities,relationships between entities  as required from the interpretation of material terms given by the next user's prompt
...
def csv2json():
    with open("2_4_1/batch_4_1.csv", "r") as fin:
        with open("2_4_1/batch_4_1.jsonl", 'w', encoding='utf-8') as fout:
...
def json2csv():
    ...
    with open("2_4_1/batch_ans_4_1.jsonl", "r") as fin:
        with open("2_4_1/batch_ans_4_1.csv", 'w', newline='', encoding='utf-8') as fout:
            rows = [columns]
            for line in fin:
...
def batch_llm():
    ...
    # 文件路径
    input_file_path = "2_4_1/batch_4_1.jsonl"  # 可替换为您的输入文件路径
    output_file_path = "2_4_1/batch_ans_4_1.jsonl"  # 可替换为您的输出文件路径
    error_file_path = "2_4_1/error_batch_ans_4_1.jsonl"  # 可替换为您的错误文件路径
    try:
...
# 按范围生成子列表
    ...
    for i in num_list:  # 2032报错未解决  2068, 8651  4248   2098(2100)没有英文释义   2144  {2068开始clean_space清除空格}  2599   4000, 4000  4302、5615、5821英文释义空 4000-4000 4000-8651   0-4000 4000-2933
        ...
        print(f"num:{i}\n")
        csv_gen(prompt)
    with open("./2_4_1/no_extract_id.pkl", "wb") as file:  # wb 表示以二进制写模式打开文件
