...
def messages_builder_example(content1,content2):
    messages = [{"role": "system", "content": "You are an expert agent who specializes in analyzing a large dictionary of material, and your task is to extract Entities, attributes of entities from given interpretations of material terms.The attributes of entities not must be one of the types in 'attribute_types' in the user list provided（The attributes of entities are looked up in the provided list whenever possible, but off-list attributes are allowed to be looked up" +
                                    "Please extract the entities、attributes of entities in the user tips and generate the output in JSON format according to the following material terms interpretation.The output should be a json list, without any extra line breaks, Spaces, or useless prompt language, " +
                                    " --> Beginning of the case of json: the extraction of entity attribute" +
                                    "If you are extracting attributes of entities, the json object must contain the following keywords:\"entity\", \"attribute \", \"value\" " +
                                    ...
                                    " --> Ending of the case: the extraction of entity attribute"}, {"role": "user", "content": content1},{"role": "user", "content": content2}]
    return messages
...
def csv_gen(p2, file_path="./2_4_2/batch_4_2.csv", custom_id=None):
    """
    ...
    prompt1 = f'''
            Based on the following explanation of attribute types and example  , extract Entities, attributes of entities as required from the interpretation of material terms given by the next user's prompt
    ...
    prompt5 = '''Here is an example of the overall extraction task
                             ...
                             # Specification ################ 
                              nitronatrite:Also known as chilean saltpeter, it is a borate mineral with island structure.  The crystal of saltpeter is rhombohedral, while the aggregate is irregular, such as granular, massive, crusty or powdery.  Saltpeter is usually white, but due to the influence of impurities, it can appear yellow or brown tone.  Saltpeter is white on streaks and has glass luster, usually transparent to translucent.  It has some unique chemical properties, easy to absorb water in the air and deliquescence, and also easy to dissolve in water.  Saltpeter is mainly distributed in hot and arid desert areas, and often coexists with other minerals such as gypsum, mirabilite, salt, etc.  Saltpeter is an important raw mineral material for manufacturing chemicals such as nitrogen fertilizer, nitric acid, explosive and nitrides.." +
                                # Output
                                   [{"entity": "nitronatrite", "attribute": "crystalsystem", "value": "rhombohedral"},
{"entity": "nitronatrite", "attribute": "color", "value": "white"},
{"entity": "nitronatrite", "attribute": "color", "value": "yellow"},
{"entity": "nitronatrite", "attribute": "color", "value": "brown"},
{"entity": "nitronatrite", "attribute": "color", "value": "white, yellow or brown"},
{"entity": "nitronatrite", "attribute": "shape", "value": "granular"}
{"entity": "nitronatrite", "attribute": "shape", "value": "massive"}
{"entity": "nitronatrite", "attribute": "shape", "value": "crusty "}
{"entity": "nitronatrite", "attribute": "shape", "value": "powdery"}
{"entity": "nitronatrite", "attribute": "shape", "value": "granular, massive, crusty or powdery"}
{“entity”:”nitronatrite”,”attribute:”structure”,”value”:”island”}
{“entity”:”nitronatrite”,”attribute:”streak”,”value”:white}
{“entity”:”nitronatrite”,”attribute:”luster”,”value”:”glass” }
{“entity”:”nitronatrite”,”attribute:”glass luster”,”value”:”transparent to translucent”}]
                             --> End of example
                             '''

    p1 = prompt1 +prompt4+prompt5
...
def csv2json():
    with open("2_4_2/batch_4_2.csv", "r") as fin:
        with open("2_4_2/batch_4_2.jsonl", 'w', encoding='utf-8') as fout:
...
def json2csv():
    ...
    with open("2_4_2/batch_ans_4_2.jsonl", "r") as fin:
        with open("2_4_2/batch_ans_4_2.csv", 'w', newline='', encoding='utf-8') as fout:
            rows = [columns]
            for line in fin:
...
def batch_llm():
    ...
    # 文件路径
    input_file_path = "2_4_2/batch_4_2.jsonl"  # 可替换为您的输入文件路径
    output_file_path = "2_4_2/batch_ans_4_2.jsonl"  # 可替换为您的输出文件路径
    error_file_path = "2_4_2/error_batch_ans_4_2.jsonl"  # 可替换为您的错误文件路径
    try:
...
# 按范围生成子列表
    ...
    for i in num_list:  # 2032报错未解决  2068, 8651  4248   2098(2100)没有英文释义   2144  {2068开始clean_space清除空格}  2599   4000, 4000  4302、5615、5821英文释义空 4000-4000 4000-8651   0-4000 4000-2933
        ...
        print(f"num:{i}\n")
        csv_gen(prompt)
    with open("./2_4_2/no_extract_id.pkl", "wb") as file:  # wb 表示以二进制写模式打开文件
