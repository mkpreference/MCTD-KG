...
def csv_gen(p2, file_path="./2_2_1/batch_2_1.csv", custom_id=None):
    """
    ...
    prompt1 = f'''
            Based on the following explanation of relation types , extract Entities,relationships between entities  as required from the interpretation of material terms given by the next user's prompt
...
def csv2json():
    with open("2_2_1/batch_2_1.csv", "r") as fin:
        with open("2_2_1/batch_2_1.jsonl", 'w', encoding='utf-8') as fout:
...
def json2csv():
    ...
    with open("2_2_1/batch_ans_2_1.jsonl", "r") as fin:
        with open("2_2_1/batch_ans_2_1.csv", 'w', newline='', encoding='utf-8') as fout:
            rows = [columns]
            for line in fin:
...
def batch_llm():
    ...
    # 文件路径
    input_file_path = "2_2_1/batch_2_1.jsonl"  # 可替换为您的输入文件路径
    output_file_path = "2_2_1/batch_ans_1_1.jsonl"  # 可替换为您的输出文件路径
    error_file_path = "2_2_1/error_batch_ans_2_1.jsonl"  # 可替换为您的错误文件路径
    try:
...
#         print(f"num:{i}\n")
#         csv_gen(prompt)
#     with open("./2_2_1/no_extract_id.pkl", "wb") as file:  # wb 表示以二进制写模式打开文件
