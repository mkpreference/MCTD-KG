...
def messages_builder_example(content1,content2):
    messages = [{"role": "system", "content": "You are an expert agent who specializes in analyzing a large dictionary of material, and your task is to extract Entities, attributes of entities, relationships between entities from given interpretations of material terms.The relationships must be one of the types in 'relation_types' in the user list provided.The attributes of entities not must be one of the types in 'attribute_types' in the user list provided（The attributes of entities are looked up in the provided list whenever possible, but off-list attributes are allowed to be looked up）" +
                                    ...
                                    " --> Beginning of the second case of json: the extraction of entity attribute" +
                                    ...
                                    " --> Ending of the second case: the extraction of entity attribute"}, {"role": "user", "content": content1},{"role": "user", "content": content2}]
    return messages
...
def csv_gen(p2, file_path="./1_2/batch_2.csv", custom_id=None):
    """
    ...
    prompt1 = f'''
            Based on the following explanation of relation types and attribute types , extract Entities, attributes of entities, relationships between entities  as required from the interpretation of material terms given by the next user's prompt
            Use the following relation types:
            Extracting other triples that do not contain relation types given below is prohibited
            #relation_types
            {relation_types}
            The following is an explanation of relation types to better understand relationship types

            '''

    prompt2 = '''
            --> Beginning of explanation relationship types
          ...
          isSimilarTo: This describes the similarity between two materials, particularly in terms of their properties, characteristics, or applications.
           ...
           -->Ending of explanation relationship types
            '''
    ...
    p1 = prompt1 + prompt2 +prompt3 +prompt4
...
def csv2json():
    with open("1_2/batch_2.csv", "r") as fin:
        with open("1_2/batch_2.jsonl", 'w', encoding='utf-8') as fout:
...
def json2csv():
    ...
    with open("./1_2/batch_ans_2.jsonl", "r") as fin:
        with open("1_2/batch_ans_2.csv", 'w', newline='', encoding='utf-8') as fout:
            rows = [columns]
            for line in fin:
...
def batch_llm():
    ...
    # 文件路径
    input_file_path = "1_2/batch_2.jsonl"  # 可替换为您的输入文件路径
    output_file_path = "1_2/batch_ans_2.jsonl"  # 可替换为您的输出文件路径
    error_file_path = "1_2/error_batch_ans_2.jsonl"  # 可替换为您的错误文件路径
    try:
...
# 按范围生成子列表
    ...
    for i in num_list:  # 2032报错未解决  2068, 8651  4248   2098(2100)没有英文释义   2144  {2068开始clean_space清除空格}  2599   4000, 4000  4302、5615、5821英文释义空 4000-4000 4000-8651   0-4000 4000-2933
        ...
        print(f"num:{i}\n")
        csv_gen(prompt)
    with open("./1_2/no_extract_id.pkl", "wb") as file:  # wb 表示以二进制写模式打开文件
