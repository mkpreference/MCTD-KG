...
class HybridKBQAPipeline:
    ...
    def build_graph_vocabulary(self, cache_dir="/root/files_lpz/ra/实验/literature/exp_code/kgqa_tool_cache", force_rebuild=False):
        """
        ...
        cypher = """
        MATCH (n)
        WHERE (n:Term OR n:Classification) AND n.name IS NOT NULL AND n.source_doc IS NULL
...
"""
    ...
    def retrieve_subgraph(self, entities, target_relations, question, max_triplets_limit=30):
        ...
        with self.driver.session() as session:
            # ================= 通道一：抓取目标实体的全量内部属性 =================
            # === [MODIFIED] 仅查无 source_doc 属性的工具书域实体 ===
            prop_cypher = """
            MATCH (n)
            WHERE n.name IN $entities AND (n:Term OR n:Other OR n:Classification) AND n.source_doc IS NULL
            ...
            if len(entities) == 1:
                ...
                if is_pure_classification:
                    taxonomy_cypher = """
                    MATCH (n)
                    WHERE n.name = $entity AND (n:Term OR n:Other OR n:Classification) AND n.source_doc IS NULL
                    MATCH p = (n)-[:isAKindOf|subclassOf*1..4]->(m:Classification)
                    // 【核心优化】：限定 m 必须是最尽头的节点（不能再有向上的分类关系）
                    WHERE NOT (m)-[:isAKindOf|subclassOf]->(:Classification) AND m.source_doc IS NULL
                      AND ALL(x IN nodes(p) WHERE x.source_doc IS NULL)   /* === [ADD] 仅工具书域 === */
                      AND ALL(r IN relationships(p) WHERE r.source_doc IS NULL)   /* === [ADD] 仅工具书域 === */
                ...
                if len(target_relations) > 0:
                    ...
                    # === [MODIFIED] 仅查无 source_doc 属性的工具书域三元组 ===
                    core_cypher = f"""
                    ...
                    WHERE n.name = $entity AND (n:Term OR n:Other OR n:Classification) AND (m:Term OR m:Other OR m:Classification)
                      AND n.source_doc IS NULL AND m.source_doc IS NULL AND r.source_doc IS NULL
                    RETURN [x IN nodes(p) | x.name] AS path_nodes, [x IN relationships(p) | type(x)] AS path_rels
                    LIMIT 50
                    """
                ...
                MATCH p = (n)-[*1..2]-(m)
                WHERE n.name = $entity AND (n:Term OR n:Other OR n:Classification) AND (m:Term OR m:Other OR m:Classification)
                AND n.source_doc IS NULL AND m.source_doc IS NULL
                AND ALL(x IN nodes(p) WHERE x.source_doc IS NULL)   /* === [ADD] 仅工具书域 === */
                AND ALL(r IN relationships(p) WHERE r.source_doc IS NULL)   /* === [ADD] 仅工具书域 === */
                AND (
                    length(p) = 2
            ...
            # ================= 策略 B：多实体协同检索 (>= 2个实体) =================
                ...
                path_cypher = """
                MATCH (n) WHERE n.name IN $entities AND (n:Term OR n:Other OR n:Classification) AND n.source_doc IS NULL
                ...
                if core_path_nodes:
                    ...
                    if target_relations:
                        ...
                        WHERE (neighbor:Term OR neighbor:Other OR neighbor:Classification)
                          AND neighbor.source_doc IS NULL AND r.source_doc IS NULL   /* === [ADD] 仅工具书域 === */
                        RETURN [x IN nodes(p) | x.name] AS path_nodes, [x IN relationships(p) | type(x)] AS path_rels
                        """
...
if __name__ == "__main__":
    # 填入你的测试集文件路径
    evaluate_dataset("/root/files_lpz/ra/实验/literature/qa_dataset/qa_dataset_merged.json", "/root/files_lpz/ra/实验/literature/exp_code/kg_scale/tool_kgqa_evaluation_results_split.json")