...
class HybridKBQAPipeline:
    ...
    def build_graph_vocabulary(self, cache_dir="/root/files_lpz/ra/实验/literature/exp_code/kgqa_tool_cache", force_rebuild=False):
        ...
        cypher = """
        MATCH (n)
        WHERE (n:Term OR n:Classification)
        AND n.name IS NOT NULL
        AND (
        n.source_doc = 'origin'
        OR 'origin' IN n.source_doc
        )
...
"""
    ...
    def retrieve_subgraph(self, entities, target_relations, question, max_triplets_limit=30):
        ...
        with self.driver.session() as session:
            # ================= 通道一：抓取目标实体的全量内部属性 =================
            # === [MODIFIED] 实体不做 source_doc 限制，仅关系过滤 ===
            prop_cypher = """
            MATCH (n)
            ...
            if len(entities) == 1:
                ...
                if is_pure_classification:
                    ...
                    WHERE NOT (m)-[:isAKindOf|subclassOf]->(:Classification)
                      AND ALL(r IN relationships(p) WHERE r.source_doc IS NULL)   /* === [MODIFIED] 仅关系限制工具书域，实体不限 === */
                ...
                if len(target_relations) > 0:
                    ...
                    # === [MODIFIED] 仅关系限制无 source_doc，实体不限 ===
                    core_cypher = f"""
            ...
            # ================= 策略 B：多实体协同检索 (>= 2个实体) =================
                ...
                if has_classification_relation:
                    ...
                    WHERE ALL(r IN relationships(p) WHERE r.source_doc IS NULL)   /* === [MODIFIED] 仅关系限制工具书域，实体不限 === */
                    RETURN [x IN nodes(p) | x.name] AS path_nodes, [x IN relationships(p) | type(x)] AS path_rels
                    LIMIT 50
                    """
                ...
                if core_path_nodes:
                    ...
                    if target_relations:
                        ...
                        WHERE (neighbor:Term OR neighbor:Other OR neighbor:Classification)
                          AND r.source_doc IS NULL   /* === [MODIFIED] 仅关系限制工具书域，实体不限 === */
                        RETURN [x IN nodes(p) | x.name] AS path_nodes, [x IN relationships(p) | type(x)] AS path_rels
                        """
...
if __name__ == "__main__":
    # 填入你的测试集文件路径
    evaluate_dataset("/root/files_lpz/ra/实验/literature/qa_dataset/qa_dataset_merged_order.json", "/root/files_lpz/ra/实验/literature/exp_code/kg_scale/3tool_kgqa_evaluation_results_split.json")