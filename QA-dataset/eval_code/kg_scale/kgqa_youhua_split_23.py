...
class HybridKBQAPipeline:
    ...
    def build_graph_vocabulary(self, cache_dir="/root/files_lpz/ra/实验/literature/exp_code/kgqa_23_cache", force_rebuild=False):  # === [MODIFIED] 2/3 消歧实验独立缓存目录 ===
        """
        ...
        # ================= 缓存不存在或强制重建，执行原始拉取逻辑 =================
        print("[系统日志] 未使用缓存，正在从 Neo4j 提取 2/3 子图实体名称...")
        # === [MODIFIED] Term/Other 仅取 in_subgraph_2_3=true，Classification 不限 ===
        cypher = """
        MATCH (n)
        WHERE
        n.name IS NOT NULL
        AND
        (
        (n:Term AND n.in_subgraph_2_3 = true)
        OR
        n:Classification
        )
...
"""
    ...
    def retrieve_subgraph(self, entities, target_relations, question, max_triplets_limit=30):
        ...
        with self.driver.session() as session:
            # ================= 通道一：抓取目标实体的全量内部属性 =================
            # === [MODIFIED] Term/Other 仅查 in_subgraph_2_3=true，Classification 不限 ===
            prop_cypher = """
            MATCH (n)
            WHERE n.name IN $entities 
            AND ((n:Term OR n:Other) AND n.in_subgraph_2_3 = true OR n:Classification)
            ...
            if len(entities) == 1:
                ...
                if is_pure_classification:
                    ...
                    WHERE NOT (m)-[:isAKindOf|subclassOf]->(:Classification)
                      AND ALL(r IN relationships(p) WHERE (startNode(r):Classification OR endNode(r):Classification) OR r.in_subgraph_2_3 = true)   /* === [ADD] Term/Other 间关系需 2/3 子图 === */
                ...
                if len(target_relations) > 0:
                    ...
                    WHERE n.name = $entity AND (n:Term OR n:Other OR n:Classification) AND (m:Term OR m:Other OR m:Classification)
                      AND ((n:Classification OR m:Classification) OR r.in_subgraph_2_3 = true)   /* === [ADD] Term/Other 间关系需 2/3 子图 === */
                    RETURN [x IN nodes(p) | x.name] AS path_nodes, [x IN relationships(p) | type(x)] AS path_rels
                    LIMIT 50
                    """
            ...
            # ================= 策略 B：多实体协同检索 (>= 2个实体) =================
                ...
                if has_classification_relation:
                    ...
                    WHERE ALL(r IN relationships(p) WHERE (startNode(r):Classification OR endNode(r):Classification) OR r.in_subgraph_2_3 = true)   /* === [ADD] Term/Other 间关系需 2/3 子图 === */
                    RETURN [x IN nodes(p) | x.name] AS path_nodes, [x IN relationships(p) | type(x)] AS path_rels
                    LIMIT 50
                    """
                ...
                if core_path_nodes:
                    ...
                    if target_relations:
                        ...
                        WHERE (neighbor:Term OR neighbor:Other OR neighbor:Classification)
                          AND ((n:Classification OR neighbor:Classification) OR r.in_subgraph_2_3 = true)   /* === [ADD] Term/Other 间关系需 2/3 子图 === */
                        RETURN [x IN nodes(p) | x.name] AS path_nodes, [x IN relationships(p) | type(x)] AS path_rels
                        """
...
if __name__ == "__main__":
    # 填入你的测试集文件路径
    evaluate_dataset("/root/files_lpz/ra/实验/literature/qa_dataset/qa_dataset_merged.json", "/root/files_lpz/ra/实验/literature/exp_code/kg_scale/23scale_kgqa_evaluation_results_split.json")