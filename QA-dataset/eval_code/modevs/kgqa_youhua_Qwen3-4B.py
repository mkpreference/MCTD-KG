...
MODEL_NAME = "qwen-max"
ANS_GEN_MODEL = "qwen-plus"  # 备用 API 模型（本地模型不可用时切换）
# ================= [MODIFIED] Qwen3-4B 本地开源模型配置（Model Scale 实验 - 缓存复用模式） =================
LOCAL_MODEL_PATH = "/root/files_lpz/ra/实验/literature/exp_code/modevs/open_model/Qwen3-4B"  # [MODIFIED]
USE_LOCAL_ANS_MODEL = True   # True=使用本地开源模型生成答案, False=使用 API
LOCAL_MODEL_DEVICE = "cuda:1"  # [MODIFIED] 使用 GPU 1，与 Llama-3.1-8B (GPU 0) 错开
# [MODIFIED] 从已有上下文缓存加载，跳过意图提取+实体链接+子图检索步骤
LOAD_CONTEXT_FROM_CACHE = True  # [MODIFIED] True=从缓存加载上下文直接生成答案
CONTEXT_CACHE_PATH = "/root/files_lpz/ra/实验/literature/exp_code/modevs/context_cache.json"
# [MODIFIED] 缓存复用模式下无需加载 BGE-M3 向量模型，节省显存和启动时间
SKIP_EMBEDDING_MODEL = True  # [MODIFIED] 跳过 BGE-M3 加载
# ===========================================================================
...
class HybridKBQAPipeline:
    ...
    def __init__(self):
        ...
        # [MODIFIED] 缓存复用模式下跳过 BGE-M3 加载，节省 GPU 显存
        if not SKIP_EMBEDDING_MODEL:
            ...
            print("[系统日志] 缓存复用模式，跳过 BGE-M3 向量模型加载。")
        ...
        if USE_LOCAL_ANS_MODEL:
            from transformers import AutoModelForCausalLM, AutoTokenizer
            import torch
            print(f"[系统日志] 正在加载本地答案生成模型 ({LOCAL_MODEL_PATH}) 到 {LOCAL_MODEL_DEVICE}...")
            self.local_tokenizer = AutoTokenizer.from_pretrained(LOCAL_MODEL_PATH, trust_remote_code=True)
            ...
            print("[系统日志] 本地答案生成模型加载完毕。")

    def close(self):
        """关闭数据库连接"""
        self.driver.close()

    def build_graph_vocabulary(self, cache_dir="/data/li-pei-ze/lpz/ra/实验/数据集构建/cache", force_rebuild=False):
        """
        ...
        # ================= 缓存不存在或强制重建，执行原始拉取逻辑 =================
        print("[系统日志] 未使用缓存，正在从 Neo4j 提取全量实体名称...")
...
"""
        ...
        # [MODIFIED] 仅使用本地 Qwen3-4B 模型生成答案（不使用在线 API）
        if USE_LOCAL_ANS_MODEL:
            if self.local_model is None:
                raise RuntimeError("本地模型未加载，请检查 USE_LOCAL_ANS_MODEL / LOCAL_MODEL_PATH 配置")
            return self._generate_answer_local(prompt)
        # 仅当 USE_LOCAL_ANS_MODEL=False 时才使用 API
    ...
    # [MODIFIED] 本地模型答案生成方法（Qwen3-4B，chat_template，禁用思考模式直接输出答案）
    def _generate_answer_local(self, prompt):
        messages = [
            {"role": "system", "content": "你是一个基于图谱事实的严谨学术助手，绝不编造信息，遵循层级推导规则，且只输出最精简的核心答案。"},
            {"role": "user", "content": prompt}
        ]
        text = self.local_tokenizer.apply_chat_template(
            ...
            enable_thinking=False  # [MODIFIED] 禁用 Qwen3 思考模式，直接输出答案
        )
...
# ================= [MODIFIED] 自动化评测逻辑（缓存复用模式：跳过检索，仅做答案生成+评估） =================

def evaluate_dataset(dataset_path, output_path):
    """
    [MODIFIED] 缓存复用模式评测：
    1. 从 CONTEXT_CACHE_PATH 加载 Qwen3-8B 已保存的检索上下文
    2. 跳过意图提取、实体链接、子图检索
    3. 仅使用本脚本的本地模型（Qwen3-4B）生成答案
    4. 调用 Qwen API 进行答案评估
    """
    with open(dataset_path, 'r', encoding='utf-8') as f:
        qa_data = json.load(f)

    # [MODIFIED] 加载 Qwen3-8B 已保存的上下文缓存（复用检索结果）
    print(f"[系统日志] 正在加载上下文缓存: {CONTEXT_CACHE_PATH}")
    with open(CONTEXT_CACHE_PATH, 'r', encoding='utf-8') as f:
        context_cache = json.load(f)
    # 建立 id -> cache_entry 的映射
    cache_by_id = {entry["id"]: entry for entry in context_cache}
    print(f"[系统日志] 上下文缓存加载完毕，共 {len(context_cache)} 条。")

    # [MODIFIED] 初始化管道（不加载 BGE-M3，不构建词表）
    pipeline = HybridKBQAPipeline()
    # 跳过 build_graph_vocabulary()，缓存复用模式不需要

    results_log = []
    correct_count = 0

    print(f"\n[评测日志] [Qwen3-4B 缓存复用模式] 开始评测，总计 {len(qa_data)} 道题目...\n")
    ...
    for idx, item in enumerate(qa_data):
        ...
        # [MODIFIED] 从缓存获取检索上下文（跳过整个检索管道）
        cached = cache_by_id.get(qid)
        if cached is None:
            print(f"[警告] {qid} 在上下文缓存中未找到，跳过。")
            continue
        retrieved_context = cached["retrieved_context"]
        raw_mentions = cached.get("raw_mentions", [])
        linked_entities = cached.get("linked_entities", [])
        target_relations = cached.get("target_relations", [])

        print(f"[{qid}] 正在处理问题: {question}")

        # [MODIFIED] 直接生成答案（不调用 run_pipeline，只调用 generate_answer）
        llm_answer = pipeline.generate_answer(question, retrieved_context)
        ...
        print(f"   -> 缓存实体 : {linked_entities}")
        print(f"   -> 缓存关系 : {target_relations}")
        print(f"   -> 模型预测结果 : {llm_answer} (标准答案: {gold_answer})")
        print(f"   -> 评测状态     : {'[通过]' if is_correct else '[失败]'}\n")

        # 保存详细的推理过程日志
        item.update({
            "pipeline_mentions": raw_mentions,
            "pipeline_linked": linked_entities,
    ...
    final_output = {
        "experiment_summary": {
            "model": "Qwen3-4B",  # [MODIFIED]
            "mode": "cache_reuse",  # [MODIFIED]
...
if __name__ == "__main__":
    evaluate_dataset(
        "/root/files_lpz/ra/实验/literature/qa_dataset/qa_dataset_merged.json",
        "/root/files_lpz/ra/实验/literature/exp_code/modevs/qwen3_4b_evaluation_results.json"  # [MODIFIED]
    )