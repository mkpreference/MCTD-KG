...
# ================= [MODIFIED] Llama-3.1-8B 本地开源模型配置（Model Scale 实验 - 缓存复用模式） =================
LOCAL_MODEL_PATH = "/root/files_lpz/ra/实验/literature/exp_code/modevs/open_model/Llama-3.1-8B-Instruct"  # [MODIFIED]
USE_LOCAL_ANS_MODEL = True   # True=使用本地开源模型生成答案, False=使用 API
LOCAL_MODEL_DEVICE = "cuda:0"  # 本地模型加载的设备
...
"""
        ...
        # [MODIFIED] 仅使用本地 Llama-3.1-8B 模型生成答案（不使用在线 API）
        if USE_LOCAL_ANS_MODEL:
    ...
    # [MODIFIED] 本地模型答案生成方法（Llama-3.1-8B，chat_template 无思考模式）
...
def evaluate_dataset(dataset_path, output_path):
    ...
    2. 跳过意图提取、实体链接、子图检索
    3. 仅使用本脚本的本地模型（Llama-3.1-8B）生成答案
    ...
    print(f"\n[评测日志] [Llama-3.1-8B 缓存复用模式] 开始评测，总计 {len(qa_data)} 道题目...\n")
    ...
    final_output = {
        "experiment_summary": {
            "model": "Llama-3.1-8B-Instruct",  # [MODIFIED]
...
if __name__ == "__main__":
    evaluate_dataset(
        ...
        "/root/files_lpz/ra/实验/literature/exp_code/modevs/llama3_1_8b_evaluation_results.json"  # [MODIFIED]
    )