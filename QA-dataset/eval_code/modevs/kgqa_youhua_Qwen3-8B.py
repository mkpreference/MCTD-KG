...
# ================= [ADD] 本地开源模型配置（Model Scale 实验） =================
LOCAL_MODEL_PATH = "/root/files_lpz/ra/实验/literature/exp_code/modevs/open_model/Qwen3-8B"
USE_LOCAL_ANS_MODEL = True   # True=使用本地开源模型生成答案, False=使用 API
LOCAL_MODEL_DEVICE = "cuda:0"  # 本地模型加载的设备（不能和 BGE-M3 同卡）
# [ADD] 上下文缓存路径（保存中间检索结果，供其他模型复用）
...
"""
    ...
    def _generate_answer_local(self, prompt):
        ...
        text = self.local_tokenizer.apply_chat_template(
            ...
            enable_thinking=False  # 禁用 Qwen3 思考模式，直接输出答案
        )
...
def evaluate_dataset(dataset_path, output_path):
    ...
    results_log = []
    context_cache = []  # [ADD] 保存中间上下文供其他模型复用
    correct_count = 0
    ...
    for idx, item in enumerate(qa_data):
        ...
        context_cache.append({
            ...
            "target_relations": result["target_relations"],
        })
    ...
    with open(CONTEXT_CACHE_PATH, 'w', encoding='utf-8') as f:
        json.dump(context_cache, f, ensure_ascii=False, indent=2)
    print(f"[系统日志] 上下文缓存已保存至 {CONTEXT_CACHE_PATH}，共 {len(context_cache)} 条")

if __name__ == "__main__":
    evaluate_dataset(
        "/root/files_lpz/ra/实验/literature/qa_dataset/qa_dataset_merged.json",
        "/root/files_lpz/ra/实验/literature/exp_code/modevs/qwen3_8b_evaluation_results.json"
    )